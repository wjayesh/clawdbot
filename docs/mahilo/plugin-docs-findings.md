...
## Findings
### Critical
- [CRIT-1] The manifest and lifecycle guidance are incompatible with the Clawdbot plugin system. The doc models `moltbot.plugin.json` (legacy `clawdbot.plugin.json`) as a runtime registry with `tools`, `routes`, and `hooks`, and omits required `id` + `configSchema`, which will fail config validation and prevent plugin load. `docs/mahilo/plugin-design.md:222` `docs/mahilo/tasks-plugin.md:37` `docs/plugins/manifest.md:16` `src/plugins/loader.ts:271`
...
- [HIGH-4] Tool examples import `ClawdbotPluginApi`, but the plugin SDK exports `MoltbotPluginApi`, so the snippets will not type-check for external plugins. `docs/mahilo/plugin-design.md:389` `src/plugin-sdk/index.ts:1`
